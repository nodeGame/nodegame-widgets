...
(function(node) {
    ...
    node.widgets.register('Network', Network);

    // ## Meta-data

    Network.version = '0.1.0';
    Network.description = 'Creates a network layout of other players to interact with.';

    Network.title = 'Done Button';
    Network.className = 'Network';
    Network.texts.done = 'Done';
    ...
    function Network(options) {
        ...
        if ('object' === typeof options.button) {
            ...
            throw new TypeError('Network constructor: options.button must ' +
                                'be object or undefined. Found: ' +
                                options.button);
        }
     ...
     * - className: the className of the button (string, array), or false
     *     to have none. Default bootstrap classes: 'btn btn-lg btn-primary'
     * - text: the text on the button. Default: Network.text
     *
     * @param {object} options Optional. Configuration options
     */
    Network.prototype.init = function(options) {
        var tmp;
        options = options || {};

        //Button
        if ('undefined' === typeof options.id) {
            tmp = Network.className;
        }
        else if ('string' === typeof options.id) {
            tmp = options.id;
        }
            ...
            throw new TypeError('Network.init: options.id must ' +
                                'be string, false, or undefined. Found: ' +
                                options.id);
        }
        this.button.id = tmp;
        ...
        if ('undefined' === typeof options.className) {
            ...
            throw new TypeError('Network.init: options.className must ' +
                                'be string, array, or undefined. Found: ' +
                                options.className);
        }
        this.button.className = tmp;
    ...
    Network.prototype.append = function() {
        this.bodyDiv.appendChild(this.button);
    };

    Network.prototype.listeners = function() {
        var that = this;

        // This is normally executed after the PLAYING listener of
        // GameWindow where lockUnlockedInputs takes place.
        // In case of a timeup, the Network will be locked and
        // then unlocked by GameWindow, but otherwise it must be
        // done here.
        node.on('PLAYING', function() {
            var prop, step;
            step = node.game.getCurrentGameStage();
            prop = node.game.plot.getProperty(step, 'Network');
            if (prop === false || (prop && prop.enableOnPlaying === false)) {
                // It might be disabled already, but we do it again.
                that.disable();
            }
     ...
     */
    Network.prototype.setText = function(text) {
        if ('undefined' === typeof text) {
            text = Network.text;
        }
        else if ('string' !== typeof text) {
            throw new TypeError('Network.setText: text must ' +
                                'be string or undefined. Found: ' +
                                typeof text);
        }
        this.button.value = text;
